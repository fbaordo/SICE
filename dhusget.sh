...
if [ -z $TIME ] && [ -z $TIMEFILE ] && [ -z $ROW ] && [ -z $PRODUCT_TYPE ] && [ -z $FREE_SUBQUERY_CHECK ] && [ -z $SENSING_TIME ] && [ -z $MISSION ] && [ -z $INSTRUMENT ]; then
	echo ""
...
if [[ ! -z $x1 ]]; then
	...
	export GEO_SUBQUERY=$(
		...
		printf "( footprint:\"Intersects(POLYGON((%.13f %.13f,%.13f %.13f,%.13f %.13f,%.13f %.13f,%.13f %.13f )))\")" $x1 $y1 $x2 $y1 $x2 $y2 $x1 $y2 $x1 $y1
	)
...
if [ ! -z $TIMEFILE ]; then
	if [ $(cat "${NAMEFILERESULTS}" | grep '="ingestiondate"' | head -n 1 | cut -f2 -d'>' | cut -f1 -d'<' | wc -l) -ne 0 ]; then
...
if [ "${TO_DOWNLOAD}" == "product" -o "${TO_DOWNLOAD}" == "all" ]; then
	...
	while [[ -f ${TMP_OFFLINE_PRODUCT_LIST} ]] && [[ ${OFFLINE_RETRY_ATTEMPTS_COUNTER} -lt ${OFFLINE_RETRY_ATTEMPTS} ]]; do
		...
		cat "${TMP_OFFLINE_PRODUCT_LIST}" | while IFS=, read idx uuid name; do
